
function mostrarParte2(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'block';
}
function mostrarParte3(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'block';
 
}
function mostrarParte4(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'block';
   
}

function mostrarParte5(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'block';
    
}
function mostrarParte6(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'block'; 
}

function mostrarParte7(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'block';

}

function mostrarParte8(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'none';
    document.getElementById('part-8').style.display = 'block';

}
function mostrarParte9(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'none';
    document.getElementById('part-8').style.display = 'none';
    document.getElementById('part-9').style.display = 'block';
}
function mostrarParte10(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'none';
    document.getElementById('part-8').style.display = 'none';
    document.getElementById('part-9').style.display = 'none';
    document.getElementById('part-10').style.display = 'block';
}
function mostrarParte11(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'none';
    document.getElementById('part-8').style.display = 'none';
    document.getElementById('part-9').style.display = 'none';
    document.getElementById('part-10').style.display = 'none';
    document.getElementById('part-11').style.display = 'block';
}
function mostrarParte12(){
    document.getElementById('part-1').style.display = 'none';
    document.getElementById('part-2').style.display = 'none';
    document.getElementById('part-3').style.display = 'none';
    document.getElementById('part-4').style.display = 'none';
    document.getElementById('part-5').style.display = 'none';
    document.getElementById('part-6').style.display = 'none';
    document.getElementById('part-7').style.display = 'none';
    document.getElementById('part-8').style.display = 'none';
    document.getElementById('part-9').style.display = 'none';
    document.getElementById('part-10').style.display = 'none';
    document.getElementById('part-11').style.display = 'none';
    document.getElementById('part-12').style.display = 'block';
}


function mostrarInput(){
    document.getElementById('part-input').style.display = 'block'
}

function ocultarInput(){
    document.getElementById('part-input').style.display = 'none'
}
